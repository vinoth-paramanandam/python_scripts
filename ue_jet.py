...
def lin_interior_eqns(*data):
    ...
    splus = delta*aplus*aplus*vplus/yplus 
    sminus = delta*aminus*aminus*vminus/yminus

    tplus = qplus*uplus + rplus*vplus - splus*xplus
    tminus = qminus*uminus + rminus*vminus - sminus*xminus
    
    A = np.array([[-lamdaplus, 1.0, 0.0, 0.0], [-lamdaminus, 1.0, 0.0, 0.0], [-splus, 0.0, qplus, rplus], [-sminus, 0.0, qminus, rminus]])
...
def lin_axis_eqns(*data):
    ...
    x[1] = (tminus + sminus*(x[0] - xminus))/qminus
...
if __name__ == "__main__":
    ...
    g = 1.4
    R = 287.14
    # delta = 1 for axisymmetric jet and 0 for 2D planar jet
    delta = 0.0
    ...
    p0 = pj*(1 + (g-1.0)*0.5*Me**2)**(g/(g-1.0))
...
# Initial conditions for the various expansion waves 
    ...
    # Calculation of the first level of interaction
    for i in range(waves):
        for j in range(waves):
            ...
            if i == j:
                ...
                pass
    # Calculation of second level of interaction with the jet boundary
