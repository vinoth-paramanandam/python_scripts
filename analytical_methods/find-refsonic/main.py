...
from transitionpts import vonNuemann, detachment, sonic, threeshock_sonic
...
def flowpts(*data):
    ...
    # I0 = ([2*Mguess, 0.5, np.pi/4, np.pi/4, np.pi/2, np.pi/8, np.pi/8, np.pi/8, zd[3], 2*pguess, 2.5*pguess, 1.8*Mguess])
    I0 = ([2*Mguess, 0.5, np.pi/4, np.pi/4, np.pi/2, pguess, 2*pguess, 2.5*pguess, 1.8*Mguess])
    zrefs = fsolve(threeshock_sonic, I0, args=data)
...
if __name__ == "__main__":
    ...
    # f1 = open('M2_LnBMach.csv', 'a')
    # # f2 = open('M5_mLnB.csv', 'a')
    # # f3 = open('M3_Mou.csv', 'a')
