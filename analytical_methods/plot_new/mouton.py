...
from geometry import LinBendor
...
def Mouton(I0, *data):
    ...
    f[1] = et*np.sin(beta2- theta1) + sh - ef*np.sin(mu_MD + theta3) - sstar
    f[2] = oe*np.cos(theta1) + ef*np.cos(mu_MD + theta3) - ot*np.cos(beta1) - (sh - sstar)/np.tan(theta3)
...
def machstem(*data):
    ...
    zmachstemlnb = fsolve(LinBendor, I0, args=data)
    zmachstemmouton = fsolve(Mouton, I0[0:6], args=data)
    return zmachstemmouton, zmachstemlnb
...
if __name__ == "__main__":
    ...
    nprval = np.arange(npr, npr_vn, 0.25)
    f2 = open('newM2_LnB.csv', 'a')
    # f3 = open('newM5_mMou.csv', 'a')
    ...
    for npr in nprval:
        ...
        zMouton, zLnB = machstem(*(H, mu_MD, beta1, theta1, beta2, mu_M2, theta3, g, Mbar))
        
        # valMou = np.append(np.append([M], [npr]),
        #                    np.append(zMouton, [theta3]))

        valMou = np.append([npr], zMouton[4])
        valLnb = np.append([npr], zLnB[1])

        np.savetxt(f2, [valLnb], delimiter=",", fmt='%f')
        # np.savetxt(f3, [valMou], delimiter=',', fmt='%f')
